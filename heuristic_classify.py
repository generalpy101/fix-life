...
EXCLUDED_PROCESSES = {'chrome.exe', 'teams.exe', 'explorer.exe', 'discord.exe', 'steam.exe'}

class HeuristicClassifier:
    SCORE_THRESHOLD = 3.0  # Threshold for classifying a process as a game
    def __init__(self, process_list: List[psutil.Process], exclude: List[str] = EXCLUDED_PROCESSES):
        ...
        try:
            ...
            for gpu in gpu_info:
                ...
                if "valorant" in name.lower() or "unreal" in name.lower():
                    print(f"[GPU-WMI] Found GPU: {name} with Utilization: {gpu.UtilizationPercentage}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        for proc in all_procs:
            ...
            if label == "game":
                print(f"Process: {proc.name()}, PID: {proc.pid}, Label: {label}, Score: {score:.2f}")
    except Exception as e:
